...
**Raspberry Pi Imager**를 다운로드 받아 설치해서 실행하자. 이미지로 옮길 Raspberry Pi 는 (recommended) OS를 고르고, 설치할 SD Card의 위치를 선택해 **Write** 하자.
(시간이 좀 많이 걸린다.)
...
운영체제가 설치되면 자동으로 설정화면이 뜨는데, 여기서 와이파이를 잡아주자. 단, 지역을 KOR로 설정하는 경우 와이파이가 잡히지 않는다.

Localization 설정에서 **WiFi-country**를 반드시 **US**로 해주자.
{: .notice--warning}

또한, 5 GHz WiFi SSID 나, Hidden SSID는 라즈베리파이에서 찾지도 못하고 접속도 못하니 유의하자. 
{: .notice--warning}
...
`ifconfig`로 `wlan0` 인터페이스의 IP 주소를 확인하고, putty를 사용해 해당 주소로 접속하자.
...
DHCP 설정 및 관련 패키지 설치를 묻는 창에는 **NO**를 선택한다.
